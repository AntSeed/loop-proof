...
test("aggregate proving quote requires exact digest approval", () => {
  ...
  assert.equal(quote.body.aggregateMaxCostUsd, "43.750000");
  assert.throws(() => approveCostQuote(quote, `0x${"0".repeat(64)}`, { p0Claims: 35 }, now), /explicit approval/);
  assert.equal(approveCostQuote(quote, quote.digest, { p0Claims: 35 }, now), quote);
});

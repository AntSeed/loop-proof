...
export function buildCoverageReport(bundle, plan, results = null) {
  ...
  const resultEntries = new Map((results?.entries ?? []).map((entry) => [entry.sourceClaimId ?? entry.claimId, entry]));
  ...
  for (const [claimId, planned] of plannedClaims) {
    ...
    if (result) validateResultVolume(result, planned, selectedVolume);
    perClaim.push({
      claimId,
...
function validateResultEntry(result, planned, securityMode) {
  ...
  if (result.sourceClaimId != null && result.sourceClaimId !== planned.claimId) {
    throw new Error(`${planned.claimId}: evidence-bound result is linked to the wrong source claim`);
  }
}

function validateResultVolume(result, planned, selectedVolume) {
  const metrics = result.metrics ?? {};
  const proven = planned.type === "P0_RECIPROCAL"
    ? BigInt(metrics.volumeAToBRaw) + BigInt(metrics.volumeBToARaw)
    : BigInt(metrics.qualifiedVolumeRaw);
  if (proven !== selectedVolume) throw new Error(`${planned.claimId}: result volume differs from authenticated selected evidence`);
}

//! P0_RECIPROCAL guest: the compiled binary's verification key IS the rule.
...
fn main() {
    ...
    let input: wash_predicate::ReciprocalInput =
        serde_json::from_slice(&bytes).expect("decode reciprocal input");
    let journal =
        wash_predicate::verify_reciprocal(&input).expect("reciprocal predicate not satisfied");
